...
#     """
#     A rich transcript has a specific JSON format. This function will read in one of these transcripts and spit out a pandas dataframe
#     with the necessary keys specified

#     Args:
#         path (str): path to JSON file to open
...
#     Returns:
#         pd.Dataframe: A pandas dataframe containing the specified keys, duplicating necessary adiitional keys across rows
...
#     """
#     if bool(set(additional_doc_keys) & set(additional_utt_keys)):
#         raise ValueError("Keys must be unique between additional_doc_keys and additional_utt_keys.")

#     with open(path, 'r') as f:
#         data = json.load(f)
#         if text_key not in data.keys():
#             # TODO - inconsistent with exceptions for other keys. Shouldn't be a KeyError?
#             raise ValueError(
#                 "{} not a valid text key in dataset json at {}".format(text_key, path))
#         row_list = []
        
#         doc_keys = {}
#         approved_doc_keys = []
#         approved_utt_keys = set()

#         for key in additional_doc_keys:
#             try:
#                 doc_keys[key] = data[key]
#                 approved_doc_keys.append(key)
#             except KeyError:
#                 continue
#         try:
...
#             if evaluate:
#                 row["id"] = utt_obj["id"]
#             for key in additional_utt_keys:
#                 try:
#                     row[key] = utt_obj[key]
#                     approved_utt_keys.add(key)
#                 except KeyError:
#                     continue
#             for key in approved_doc_keys:
#                 row[key] = doc_keys[key]
#             row_list.append(row)

#         approved_utt_keys = list(approved_utt_keys)
#         df_cols = [text_key, label_key, "id"] if evaluate else [text_key, label_key]
#         df_cols.extend(approved_doc_keys)
#         df_cols.extend(approved_utt_keys)
